...
class VisitCreateRequest(BaseModel):
    ...
    world_heritage_id:  int = Field(
         ...,
         ge=1,
         description="世界遺産ID（world_heritages.id）"
         )
    visited_from: Optional[date] = Field(
         None,
         description="訪問開始日（未入力可）"
         )
    visited_to: Optional[date] = Field(
         None,
         description="訪問終了日（未入力可）"
         )

    @model_validator(mode="after")
    def validate_date_range(self) -> "VisitCreateRequest":
        """両方あるときだけ visited_from <= visited_to をチェックする"""
        if self.visited_from and self.visited_to and self.visited_from > self.visited_to:
...
class VisitsMeResponse(BaseModel):
    ...
    visited_heritage_ids: List[int] = Field(
         ...
         description="ログインユーザーが訪問済みの世界遺産ID一覧"
         )
    count: int = Field(
        0,
        description="訪問済み世界遺産の件数"
    )
